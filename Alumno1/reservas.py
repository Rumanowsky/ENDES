...
class Clientes(Personas):
    def __init__(self, nombre, apellido, telefono, identificador, saldo=0, tiempo_membresia=0):
    ...
    def incrementar_membresia(self):
        self.tiempo_membresia += 1
        print(f"Aumentando membresía: {self.nombre} ahora tiene {self.tiempo_membresia} años de membresía.")

    def aplicar_descuento(self):
        if self.tiempo_membresia >= 1:
            descuento = 0.05 * self.tiempo_membresia  
            return descuento
        return 0
...
class Sala:
    def __init__(self, numero_sala, disponible=True):
        self.numero_sala = numero_sala
        self.disponible = disponible
        self.reservas = []

    def __str__(self):
        return f"Sala {self.numero_sala}, Disponible: {'Sí' if self.disponible else 'No'}"

    def reservar_sala(self, cliente):
        if self.disponible:
            self.reservas.append(cliente)
            self.disponible = False
            print(f"Sala {self.numero_sala} reservada exitosamente por {cliente.nombre}.")
        else:
            print(f"Sala {self.numero_sala} no está disponible actualmente.")

    def comprobar_reserva_anterior(self, cliente):
        if self.reservas and self.reservas[-1] == cliente:
            return True
        return False
...
class Centro:
    ...
    def reservar_sala_entreno(self):
        numero_sala = input("Dime el número de la sala: ")
        cliente_id = input("Dime el identificador del cliente: ")

        cliente = next((cliente for cliente in self.lista_clientes if cliente.identificador == cliente_id), None)
        sala = next((sala for sala in self.lista_salas if sala.numero_sala == int(numero_sala)), None)

        if not sala or not cliente:
            print("Sala o cliente no encontrado.")
            return
        
        if sala.comprobar_reserva_anterior(cliente):
            print("No puedes reservar la misma sala dos veces seguidas.")
            return

        sala.reservar_sala(cliente)
